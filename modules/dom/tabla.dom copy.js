 ...
 export const TablaDOM = {
     ...
     */
        renderizar(filas) {
            ...
            thead.innerHTML = `<tr>${CABECERAS_LOCALES_PROPIEDADES.map(h => `<th>${h}</th>`).join('')}<th class='text-center'>Acciones</th></tr>`;

            if (filas.length === 0) {
                tbody.innerHTML = `<tr><td colspan="${CABECERAS_LOCALES_PROPIEDADES.length + 1}" style="text-align:center; padding: 20px; color: #6b7280;">No se encontraron tickets coordinados.</td></tr>`;
